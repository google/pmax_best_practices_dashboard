...
FROM `{bq_dataset}.assetgroupsummary` AS AGS
  JOIN VIDEO_DATA AS V USING (account_id, campaign_id, asset_group_id)
  JOIN TEXT_DATA AS T USING (account_id,campaign_id, asset_group_id)
  JOIN IMAGE_DATA AS I USING (account_id,campaign_id, asset_group_id)
...
SELECT
  ...
  IF(count_square_logos = 0, 1, 0) AS missing_square_logos,
  IF(count_landscape_logos = 0, 1, 0) AS missing_landscape_logos,
  IF(count_videos < 5, 5 - count_videos, 0) AS missing_video,
  IF(ASA.number_of_audience_signals IS NOT NULL,"Yes","X") AS audience_signals,
  OCID.ocid
FROM `{bq_dataset}.assetgroupsummary` AS AGS
  JOIN VIDEO_DATA AS V USING (account_id, campaign_id, asset_group_id)
