CREATE OR REPLACE TABLE `{bq_project}.{bq_dataset}_bq.assetgroupbpscore_${format(today(),'yyyyMMdd')}` AS
WITH video_data AS (
  SELECT
    account_id,
    account_name,
...
SELECT
    ...
    IF(count_long_headlines >= 1,"Yes","No") AS count_long_headlines,
    IF(count_short_descriptions >= 1,"Yes","No") AS count_short_descriptions,
    IF(count_short_headlines >= 1,"Yes","No") AS count_short_headlines,
    IF(count_images >= 15,"Yes","No") AS count_images,
    IF(count_logos >= 5,"Yes","No") AS count_logos,
    IF(count_rectangular >= 1,"Yes","No") AS count_rectangular,
    IF(count_rectangular_logos >= 1,"Yes","No") AS count_rectangular_logos,
    IF(count_square >= 1,"Yes","No") AS count_square,
    IF(count_square_logos >= 1,"Yes","No") AS count_square_logos,
    video_score,
    text_score,
    image_score
FROM video_data V
